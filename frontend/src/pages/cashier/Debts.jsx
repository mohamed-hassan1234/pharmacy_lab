...
const Debts = () => {
    ...
    const handlePayment = async () => {
        ...
        } catch (err) { alert(err.response?.data?.message || 'Error saving payment'); }
    };
        ...
        <div className="page-section animate-in fade-in duration-500">
                ...
                {filtered.map((debt) => (
                    <div key={debt._id} className="bg-white rounded-[2rem] p-6 shadow-md border border-slate-100 hover:shadow-xl transition-all group relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-4">
                            {debt.status === 'PARTIAL' ? (
                                <span className="bg-blue-100 text-blue-600 text-[10px] font-black px-3 py-1 rounded-full border border-blue-200 uppercase italic">Partial Paid</span>
                            ) : (
                                <span className="bg-red-100 text-red-600 text-[10px] font-black px-3 py-1 rounded-full border border-red-200 uppercase italic">Unpaid</span>
                            )}
                        </div>
                        ...
                        <div className="space-y-3 mb-6 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            ...
                            <div className="flex justify-between items-center text-emerald-600">
                                <span className="text-[10px] font-black uppercase">Already Paid:</span>
                                <span className="font-black">-{debt.paidAmount.toLocaleString()} SOS</span>
                            </div>
                            <div className="h-px bg-slate-200 my-2"></div>
                            <div className="flex justify-between items-end">
                                <div>
                                    <p className="text-[10px] font-black text-orange-600 uppercase tracking-widest leading-none">Still Owed:</p>
                        ...
                        >
                            <DollarSign size={20} /> Collect Money
                        </button>
                    </div>
                ))}
            </div>
                ...
                <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
                    <div className="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl animate-in zoom-in duration-300">
                        <div className="text-center mb-8">
                            ...
                            <h3 className="text-3xl font-black text-slate-800 tracking-tighter uppercase italic">Collect Payment</h3>
                            <p className="text-slate-400 font-bold text-sm mt-1">Paying for: <span className="text-slate-800">{payModal.customerName}</span></p>
                        </div>

                        <div className="space-y-6">
                            <div>
                                <div className="flex justify-between mb-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[.2em]">Amount Received (SOS)</label>
                                    <span className="text-[10px] font-black text-orange-600 uppercase">Max: {payModal.remainingBalance.toLocaleString()}</span>
                                </div>
                                <input
                                    type="number"
                            ...
                            <div className="flex gap-4">
                                ...
                                >
                                    <CheckCircle size={20} /> Confirm Paid
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            {filtered.length === 0 && !loading && (
                <div className="text-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
                    ...
                    <h3 className="text-2xl font-black text-slate-800 uppercase italic">Great News!</h3>
                    <p className="text-slate-400 font-bold">All customers have paid their bills. No pending debts.</p>
                </div>
            )}
        </div>
    );
